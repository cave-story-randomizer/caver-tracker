-- Configuration --------------------------------------
AUTOTRACKER_DISABLE_RACE_ROM_SUPPORT = false
AUTOTRACKER_ENABLE_DEBUG_LOGGING = false
-------------------------------------------------------

AUTOTRACKER_IS_IN_GAME = false
AUTOTRACKER_IS_IN_TRIFORCE_ROOM = false
AUTOTRACKER_IS_RACE_ROM = AUTOTRACKER_DISABLE_RACE_ROM_SUPPORT
...
function updateRaceRomCheckFromMemorySegment(segment)
    if AUTOTRACKER_DISABLE_RACE_ROM_SUPPORT then
        AUTOTRACKER_IS_RACE_ROM = (segment:ReadUInt8(0x308213) > 0)
        if AUTOTRACKER_IS_RACE_ROM then
            AUTOTRACKER_IS_IN_GAME = false
            print("Auto-tracking is force disabled on race roms")
        end
    else
...
function updateInGameStatusFromMemorySegment(segment)
    ...
    --  Disable auto-tracker when using race roms
    if AUTOTRACKER_IS_RACE_ROM then
        AUTOTRACKER_IS_IN_GAME = false
    end
...
function updateToggleItemFromByteAndFlag(segment, code, address, flag)
    ...
    if item then
        ...
        local flagTest = bit32.band(value, flag)
...
function updateToggleFromRoomSlot(segment, code, slot)
    ...
    if item then
        ...
        item.Active = bit32.btest(roomData, bit32.lshift(1, slot[2]))
    end
end
...
function updateFlute(segment)
    ...
    local fakeFlute = bit32.band(value, 0x02)
    local realFlute = bit32.band(value, 0x01)
...
function updateSectionChestCountFromRoomSlotList(segment, locationRef, roomSlots, callback)
    ...
    if location then
        ...
        for i,slot in ipairs(roomSlots) do
            ...
            if AUTOTRACKER_ENABLE_DEBUG_LOGGING then
                print(locationRef, roomData, bit32.lshift(1, slot[2]))
            end
                
            if bit32.btest(roomData, bit32.lshift(1, slot[2])) then
...
function read32BitTimer(segment, baseAddress)
    local timer = 0;
    timer = bit32.bor(timer, bit32.lshift(ReadU8(segment, baseAddress + 3), 24))
    timer = bit32.bor(timer, bit32.lshift(ReadU8(segment, baseAddress + 2), 16))
    timer = bit32.bor(timer, bit32.lshift(ReadU8(segment, baseAddress + 1), 8))
    timer = bit32.bor(timer, bit32.lshift(ReadU8(segment, baseAddress), 0))
